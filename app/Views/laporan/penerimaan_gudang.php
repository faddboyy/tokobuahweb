...
<div id="app" v-cloak class="container-fluid py-4">
        ...
        <div class="row align-items-center">
            <div class="col-md-7">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h4 class="fw-bold mb-0 text-dark">Laporan Penerimaan Gudang</h4>
                        <p class="text-muted small mb-0">Riwayat penerimaan barang dari supplier ke gudang</p>
                    </div>
                </div>
            </div>
    ...
    <div class="glass-panel p-3 mb-4 border-0 shadow-sm bg-white" style="border-radius:16px">
        <div class="row g-3 align-items-end">
            ...
            <div class="col-lg-3 col-md-12">
                ...
                <div class="input-group input-group-sm rounded-3 overflow-hidden border">
                    ...
                    <input type="text" class="form-control border-0 shadow-none"
                        placeholder="Kode, supplier, operator..."
            ...
            <div class="col-lg-2 col-md-4">
                <label class="small fw-bold text-muted mb-1">
                    <i data-lucide="truck" style="width:12px" class="me-1"></i>Supplier
                </label>
                <select class="form-select form-select-sm rounded-3 border fw-medium"
            ...
            <div class="col-lg-2 d-flex gap-2">
                <button class="btn btn-primary btn-sm w-100 fw-bold rounded-3 shadow-sm
                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="text-white-50 small">
                            ...
                            &bull; {{ uniqueSuplier(group.rows) }} supplier
                        </div>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        <thead style="background:#f8fafc">
                            <tr class="text-muted">
                                <th class="ps-4 py-3 border-0 small text-uppercase fw-bolder">Kode Penerimaan</th>
                                <th class="py-3 border-0 small text-uppercase fw-bolder">Kode PO</th>
                                <th class="py-3 border-0 small text-uppercase fw-bolder">Kode Supplier</th>
                                <!-- ① Kolom Operator digabung dengan Gudang -->
                                <th class="py-3 border-0 small text-uppercase fw-bolder">Gudang / Operator</th>
                                <th class="py-3 border-0 small text-uppercase fw-bolder">Waktu Penerimaan</th>
                                <!-- ② Header Status tetap, isi diganti badge -->
                        ...
                        <tbody>
                            ...
                            <tr v-else v-for="row in group.rows" :key="row.id"
                                ...
                                <!-- Kode PO + Supplier -->
                                <td>
                                    <div class="fw-bold font-mono text-dark" style="font-size:.78rem">
                                        {{ row.kode_po || '—' }}
                                    </div>
                                ...
                                <!-- Kode Supplier -->
                                <td>
                                    <span v-if="row.kode_supplier"
                                        class="font-mono text-warning fw-bold" style="font-size:.78rem">
                                        {{ row.kode_supplier }}
                                    </span>
                                    <div class="text-muted small">{{ row.nama_suplier || '-' }}</div>
                                </td>

                                <!-- ① Gudang + Operator digabung dalam satu cell -->
                                <td>
                                ...
                                <td>
                                    <div class="fw-semibold text-dark" style="font-size:12px">
                                        {{ formatDate(row.waktu_penerimaan) }}
                                    </div>
                                </td>

                                <!-- ② Status: Bootstrap badge kecil berwarna -->
                                ...
                                <td class="text-center pe-4">
                                    <a :href="detailUrl(row.id)"
                                        class="btn btn-sm btn-outline-primary rounded-3 px-3 py-1 fw-semibold"
    ...
    createApp({
            ...
            filteredGroups() {
                ...
                return this.groupedByGudang.map(group => {
                    const rows = group.rows.filter(r =>
                        (r.kode_penerimaan || '').toLowerCase().includes(s) ||
                        (r.kode_po || '').toLowerCase().includes(s) ||
                        (r.kode_supplier || '').toLowerCase().includes(s) ||
        ...
        methods: {
            async load() {
                this.loading = true;
                try {
                    const res = await axios.get('<?= base_url('laporan/penerimaan-gudang/list') ?>', {
                        params: this.filter,
                    });
                    this.allRows = res.data.data;
                    this.gudangList = res.data.gudang_list;
                    this.suplierList = res.data.suplier_list;
                    this.page = 1;
            ...
            detailUrl(id) {
                return `<?= base_url('laporan/penerimaan-gudang/detail') ?>/${id}`;
            },
            uniqueSuplier(rows) {
                return new Set(rows.map(r => r.suplier_id).filter(Boolean)).size;
            },
            ...
            // ② Badge class Bootstrap — menggantikan status-chip custom
            statusBadgeClass(s) {
                return {
            ...
            exportCSV() {
                ...
                const headers = [
                    'Gudang', 'Kode Penerimaan', 'Kode PO', 'Supplier',
                    'Kode Supplier', 'Operator', 'Status', 'Waktu',
                ];
                const rows = [];
                ...
                a.download = `laporan_penerimaan_gudang_${new Date().toISOString().slice(0, 10)}.csv`;
                a.click();
            },
        },
